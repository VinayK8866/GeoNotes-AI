    ...
    >
      <div className="flex justify-between items-start mb-2">
        ...
        <div className="flex items-center space-x-2 flex-shrink-0">
          <button onClick={() => onEdit(note)} className="text-gray-500 dark:text-gray-400 hover:text-gray-900 dark:hover:text-white transition-colors" aria-label="Edit note"><EditIcon className="w-5 h-5" /></button>
          {canShare && <button onClick={() => onShare(note)} className="text-gray-500 dark:text-gray-400 hover:text-gray-900 dark:hover:text-white transition-colors" aria-label="Share note"><ShareIcon className="w-5 h-5" /></button>}
          {isArchivedView ? (
            <>
              <button onClick={() => onUnarchive(note)} className="text-gray-500 dark:text-gray-400 hover:text-green-500 dark:hover:text-green-400 transition-colors" aria-label="Unarchive note"><ArrowUpOnSquareIcon className="w-5 h-5" /></button>
              <button onClick={() => onDeletePermanently(note.id)} className="text-gray-500 dark:text-gray-400 hover:text-red-500 transition-colors" aria-label="Delete permanently"><TrashIcon className="w-5 h-5" /></button>
            </>
          ) : (
            <button onClick={() => onArchive(note)} className="text-gray-500 dark:text-gray-400 hover:text-yellow-500 dark:hover:text-yellow-400 transition-colors" aria-label="Archive note"><ArchiveBoxIcon className="w-5 h-5" /></button>
          )}
        </div>
      </div>
